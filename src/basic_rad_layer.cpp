...
void RadLayer::onInitialize()
{
  ...
  matchSize();  // Resize layer to match the layered costmap (usually is same as Static layer)
...
void RadLayer::updateCosts(costmap_2d::Costmap2D& master_Rad, int min_i, int min_j, int max_i,
                                          int max_j)
{
  if (!enabled_)
    return;
  ...
  for (int j = min_j; j < max_j; j++)
  {
    for (int i = min_i; i < max_i; i++)
    {
      ...
      if (costmap_[index] == NO_INFORMATION)
        continue;
      master_Rad.setCost(i, j, costmap_[index]); 
    }
  }
}
